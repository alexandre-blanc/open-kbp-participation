...
class DataLoader:
    ...
    def load_and_shape_data(self, path_to_load):
        ...
        for key in shaped_data:
            ...
            elif key =="dose":  # Files with shape
                np.put(shaped_data[key], loaded_file[key]['indices'], loaded_file[key]['data'])
                # normalisation des images
                shaped_data[key] = shaped_data[key] / 100
            else:  # Files with shape
                np.put(shaped_data[key], loaded_file[key]['indices'], np.minimum(6000 , loaded_file[key]['data']) )
                # normalisation des images
                shaped_data[key] = shaped_data[key] - shaped_data[key].mean()
                shaped_data[key] = shaped_data[key] / shaped_data[key].max()
